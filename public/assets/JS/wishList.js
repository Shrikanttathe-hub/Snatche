document.querySelectorAll('.wishlist').forEach(button => {
  button.addEventListener('click', () => {
    ...
    let favList = JSON.parse(localStorage.getItem('snatchList')) || [];
...
document.addEventListener('DOMContentLoaded', () => {
  const favList = JSON.parse(localStorage.getItem('snatchList')) || [];

  document.querySelectorAll('.product-box').forEach(productBox => {
    const title = productBox.dataset.title;
    const button = productBox.querySelector('.wishlist');
